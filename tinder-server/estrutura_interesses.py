...
def lista_matches(id_pessoa):
    #pessoa_gosta = database['interesses'][id_pessoa]
    pessoa_gosta = consulta_interesses(id_pessoa)
    resposta = []
    for id_outro in pessoa_gosta:
        lista_outro = consulta_interesses(id_outro)
        if id_pessoa in lista_outro:
